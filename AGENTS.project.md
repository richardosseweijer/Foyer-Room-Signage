...
Local displays: one **sway** seat on tty1. F1–F4: dual Setup pickers (same-output reject) + dual Chromium + hardening/lab checklist. Room panel loads Relay control UI from site `relayUrl` (`FOYER_ROOM_PANEL_URL`); do **not** start Relay’s `relay-kiosk`. See INSTALL.md §7b / §7c / ARCHITECTURE.md §4a.
