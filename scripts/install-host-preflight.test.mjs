...
test("foyer_node_major parses v22.19.0 → 22", () => {
  const r = runPreflightBash('foyer_node_major "v22.19.0"; echo');
  assert.equal(r.status, 0, r.stderr);
  assert.equal(r.stdout.trim(), "22");
});

test("foyer_preflight_node_npm fails when unit PATH has Node < 22", () => {
  const dir = mkdtempSync(join(tmpdir(), "foyer-fake-node20-"));
  try {
    const bin = join(dir, "bin");
    mkdirSync(bin, { recursive: true });
    writeFileSync(join(bin, "node"), "#!/bin/sh\necho v20.19.2\n", { mode: 0o755 });
    writeFileSync(join(bin, "npm"), "#!/bin/sh\necho 9.0.0\n", { mode: 0o755 });
    const r = runPreflightBash("foyer_preflight_node_npm", {
      FOYER_UNIT_PATH: bin,
      FOYER_PREFLIGHT_NPM: join(bin, "npm"),
    });
    assert.notEqual(r.status, 0);
    assert.match(r.stderr, /preflight failed/);
    assert.match(r.stderr, /v20\.19\.2/);
    assert.match(r.stderr, /need major >= 22|major >= 22/);
    assert.match(r.stderr, /INSTALL\.md §2|NodeSource/);
...
test("foyer_preflight_build fails when nitro.json missing", () => {
  const dir = mkdtempSync(join(tmpdir(), "foyer-preflight-"));
  try {
    const r = runPreflightBash(`foyer_preflight_build "${dir}"`);
    assert.notEqual(r.status, 0);
    assert.match(r.stderr, /missing build output/);
    assert.match(r.stderr, /npm ci --include=dev && npm run build/);
    assert.match(r.stderr, /INSTALL\.md §4/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("foyer_preflight_build OK when marker present", () => {
  const dir = mkdtempSync(join(tmpdir(), "foyer-preflight-"));
  try {
...
test("foyer_preflight_node_npm OK with fake PATH containing node 22 + npm", () => {
  const dir = mkdtempSync(join(tmpdir(), "foyer-fake-node-"));
  try {
    const bin = join(dir, "bin");
    mkdirSync(bin, { recursive: true });
    writeFileSync(
