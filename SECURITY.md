...
| Kiosk | Setup session. Restarts a **fixed** unit name (`foyer-kiosk.service`) only. One sway seat; Welcome and/or Room-panel Chromium from Setup DRM picks (same-output reject). Room panel URL is site `relayUrl` written into `foyer-kiosk.env` — Setup-gated fixed units only ([INSTALL.md §7b](INSTALL.md)). |
