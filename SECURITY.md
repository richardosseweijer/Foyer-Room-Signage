...
| Kiosk | Setup session. Restarts a **fixed** unit name (`foyer-kiosk.service`) only. Today: one Welcome head (DRM scan pick). Planned multi-display Path B will keep Setup-gated fixed units — not live yet ([INSTALL.md §7b](INSTALL.md)). |
