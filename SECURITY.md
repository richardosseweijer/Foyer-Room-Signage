...
| Relay | HMAC GET `/api/peer` from `src/lib/foyer/relay.ts` on **loopback**. Prefer first-class `occupancy` bound by `room.name`. Secret stays in `foyer-secrets.json`. Occupancy snapshot only on the frame. No occupancy POST. |
